...
$C_13$ is the smallest $n$ such the value of the busy beaver number $BB(n)$ is undecidable in ZFC (or equivalently ZF). Explicitly, it is the smallest $n$ such that there is a Turing machine with $n$ states for which it cannot be proven in ZFC (assuming ZFC is consistent) whether it halts or not.
